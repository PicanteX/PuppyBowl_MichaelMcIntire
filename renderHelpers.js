...
export const renderAllPlayers = (playerList) => {
  ...
  for (let i = 0; i < playerList.length; i++) {
    ...
    let pupHTML = `
      <div class="single-player-card">
        ...
        <button class="remove-button" data-id=${pup.id}>Remove Puppy!</button>
      </div>
    `;
  ...
  for (let i = 0; i < detailButtons.length; i++) {
    ...
    button.addEventListener('click', async () => {
      let player = await fetchSinglePlayer(button.dataset.id)
  ...
  for (let i = 0; i < removeButtons.length; i++) {
    const button1 = removeButtons[i];
    button1.addEventListener('click', async () => {
      await removePlayer(button1.dataset.id);
...
export const renderSinglePlayer = (playerObj) => {
  ...
  let allPlayersReturn = [...document.getElementsByName('see-all')];
  for (let i = 0; i < allPlayersReturn.length; i++) {
    const button = allPlayersReturn[i];
    button.addEventListener('click', async () => {
      // let player = fetchSinglePlayer(button1.dataset.id)
      // renderSinglePlayer(player);
      const players = await fetchAllPlayers();
